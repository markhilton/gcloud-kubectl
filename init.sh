...
if [[ ! -f "$SSH_KEY"   ]] ; then echo -e "\033[1;91mERROR:\033[0m SSH public key file not found" && ERROR=1; fi
if [[ ! -f "$KUBE_AUTH" ]] ; then echo -e "\033[1;91mERROR:\033[0m Google service account json key file not found" && ERROR=1; fi
...
export KUBE_PROJECT=$(cat $KUBE_AUTH | jq -r .project_id)
export KUBE_ACCOUNT=$(cat $KUBE_AUTH | jq -r .client_email)
...
gcloud auth activate-service-account $KUBE_ACCOUNT --key-file=$KUBE_AUTH && \
